...
public final class UIUtils {
  ...
  public static final String HOST_SERVICES = "hostServices";

  // Declaration of PseudoClasses
  private static final PseudoClass SUCCESS_PSEUDO = PseudoClass.getPseudoClass("success");
  private static final PseudoClass INFO_PSEUDO = PseudoClass.getPseudoClass("info");
  private static final PseudoClass WARN_PSEUDO = PseudoClass.getPseudoClass("warn");
  ...
  public static void buildTree(
      ...
      Map<TreeItem<String>, TTNode<String>> treeViewMap)
      throws TTException {
   ...
   * @param <T> Controller
   */
  public static <T> void showDialog(String resource, StackPane pane, Consumer<T> consumer)
   ...
   * @param pane dialog container
   * @param consumer consumer for delivering message (via controller)
   * @param overlayClose Overlay Close flag, to prevent from being closed when click outside, set it
   *     to false
   * @param <T> Controller
   */
  public static <T> void showDialog(
      String resource, StackPane pane, Consumer<T> consumer, boolean overlayClose)
      throws IOException, NoSuchMethodException, IllegalAccessException, IllegalArgumentException,
          InvocationTargetException {
  ...
  public static String getId4FilterableTreeItem(TTNode<String> topologyNode) {
    return topologyNode.getValue() != null
        ? topologyNode.getId() + "/" + topologyNode.getValue()
        : topologyNode.getId();
  }
}
