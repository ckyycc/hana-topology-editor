...
class CommonUtilsTest {
    ...
    void getClasses_ShouldReturnClassesListWhenClassNotInJarFile() {
        List<Class<?>> classList = assertDoesNotThrow(() -> CommonUtils.getClasses(TTController.class.getPackage().getName()));
//        List<String> classNameList = classList.stream().map(Class::getName).collect(Collectors.toList());
        assertAll(() -> assertTrue(classList.contains(TTController.class)),
                () -> assertTrue(classList.contains(TTControllerImpl.class)));
    }

    @Test
    void getClasses_ShouldReturnClassesListWhenClassInJarFile() {
        List<Class<?>> classList = assertDoesNotThrow(() -> CommonUtils.getClasses(JsonReader.class.getPackage().getName()));
        assertAll(() -> assertTrue(classList.contains(JsonReader.class)),
